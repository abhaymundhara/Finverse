...
export default function RDCalculator() {
  ...
  const costOfWaiting = Math.max(maturityValue - waitFinal, 0);

  const extraIfWait = useMemo(() => {
    const investMonths = months - WAIT_MONTHS;
    if (investMonths <= 0) return Number.POSITIVE_INFINITY;
    if (monthlyRate === 0) {
      return Math.max(maturityValue / investMonths - monthlyInvestment, 0);
    }
    const factor =
      ((Math.pow(1 + monthlyRate, investMonths) - 1) / monthlyRate) *
      (1 + monthlyRate);
    const required = maturityValue / factor;
    return Math.max(required - monthlyInvestment, 0);
  }, [maturityValue, monthlyInvestment, monthlyRate, months]);

  const lowRate = Math.max(annualRate - RETURN_BUFFER_PCT, 0);
  ...
  }, [lowMonthlyRate, monthlyInvestment, months]);
  ...
  const probabilityStartNow = getProbabilityLevel({
    targetAmount: maturityValue,
    baseProjection: maturityValue,
    ...
    <div className="min-h-screen px-4 py-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-10 space-y-6">
          <StartNowVsWaitCard
            theme={{ headerGradientClassName: "from-blue-500 to-cyan-500" }}
            subtitle="Same RD installment — just skipping the first 12 deposits."
            startNowSeries={startNowSeries}
            waitSeries={waitSeries}
            goalLine={{ label: "Start-now target", value: maturityValue }}
            horizonLabels={{
              start: "Today",
              mid: `Year ${Math.floor(timeYears / 2)}`,
              end: `Year ${timeYears}`,
            }}
            stats={[
              {
              ...
              },
              {
                ...
                value: `₹${Math.round(maturityValue).toLocaleString("en-IN")}`,
              },
            ]}
            probability={{
            ...
            probabilityNote={`Rule-of-thumb: compares ${annualRate}% vs ${lowRate}% annual interest.`}
            emailCapture={{
              source: "rd",
              payload: {
                monthlyInvestment,
